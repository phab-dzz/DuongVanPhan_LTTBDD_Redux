...
const DataComponent = () => {
  ...
  return (
    <ScrollView style ={{width:"100%"}}>
      <TextInput
        placeholder="Nhap  de tao"
        value={title}
        onChangeText={setTitle}
        style={{ borderWidth: 1, marginBottom: 10, padding: 5 ,borderRadius:20}}
      />
      <Button
        color="lightblue"
        ...
        renderItem={({ item }) => (
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 10 }}>
            ...
            <View style={{ flexDirection: 'row',gap:10 }}>
              <Button color="green" title="Update" onPress={() => { setTitle(item.title); setUpdateId(item.id); }} />
